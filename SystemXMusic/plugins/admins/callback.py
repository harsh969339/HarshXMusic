...
from SystemXMusic import YouTube, app
from SystemXMusic.core.call import Sona
from SystemXMusic.misc import SUDOERS, db
from SystemXMusic.utils.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from SystemXMusic.utils.database import get_assistant
from SystemXMusic.utils.decorators.language import languageCB
from SystemXMusic.utils.formatters import seconds_to_min
from SystemXMusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from SystemXMusic.utils.stream.autoclear import auto_clean
from SystemXMusic.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Sona.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Sona.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Sona.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Sona.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
